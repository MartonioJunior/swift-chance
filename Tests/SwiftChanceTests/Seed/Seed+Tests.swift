...
struct SeedTests {
    ...
    @Test("Creates New Seed with many indices", arguments: [
        (Seed(13, position: 2, 3, 1), UInt(13), [2, 3, 1])
    ])
    func initVariadic(seed: Seed, value: UInt, position: [UInt]) async throws {
        #expect(seed.value == value)
        #expect(seed.position == position)
    }

    @Test("Creates New Seed with SIMD as position", arguments: [
        (Seed(value: 13, position: [2, 3, 1]), UInt(13), [2, 3, 1]),
        (Seed(89, position: SIMD2<UInt>(4, 5)), UInt(89), [4, 5])
    ])
    func initSIMD(seed: Seed, value: UInt, position: [UInt]) async throws {
        #expect(seed.value == value)
        #expect(seed.position == position)
    }
